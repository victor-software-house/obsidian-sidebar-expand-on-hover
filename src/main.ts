  ...
  setEvents: Function = () => {
    ...
    this.registerDomEvent(
      ...
      (event: MouseEvent) => {
        ...
        // area (.workspace-split.mod-vertical.mod-root). In overlay layouts
        // the sidebar sits on top of rootSplit as a DOM sibling, so rootSplit
        // receives mouseenter even when the pointer is visually over a sidebar.
    ...
    this.registerDomEvent(document, 'mousemove', (event: MouseEvent) => {
      ...
      if (this.isRightEdgeHovering) {
        ...
        if (target && !target.closest('.workspace-split.mod-vertical.mod-root')) {
          return; // Still over a sidebar element, not the editor
        }
